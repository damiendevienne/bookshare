...
export default function PasswordResetPage() {
  ...
  return <main className="container py-5 auth-page"><div className="row justify-content-center"><div className="col-12 col-md-6 col-lg-4"><div className="card shadow-sm text-center"><div className="card-body p-4">{invalidLink ? <><div className="display-6 mb-2">!</div><h1 className="h4">{title}</h1><p className="text-muted">{error}</p><a className="btn btn-primary w-100" href="/?login=1">Go to BookMyBook</a></> : <><h1 className="h4 mb-3">{title}</h1>{checking && <div className="text-muted">Checking your reset link…</div>}{message && <div className="alert alert-success">{message}</div>}{!checking && validLink && !message && <form onSubmit={submit} className="text-start"><label className="form-label" htmlFor="new-password">New password</label><div className="password-field mb-1"><input id="new-password" className="form-control" type={showPassword ? "text" : "password"} autoComplete="new-password" minLength="8" required value={password} onChange={(event) => setPassword(event.target.value)} /><button type="button" className="password-visibility-toggle" onClick={() => setShowPassword((current) => !current)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div><small className={`form-text d-block mb-3 ${passwordValid ? "text-success" : password ? "text-danger" : ""}`}>At least 8 characters, with letters &amp; numbers.</small><label className="form-label" htmlFor="confirm-password">Confirm password</label><div className="password-field mb-1"><input id="confirm-password" className="form-control" type={showConfirmation ? "text" : "password"} autoComplete="new-password" minLength="8" required value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.target.value)} /><button type="button" className="password-visibility-toggle" onClick={() => setShowConfirmation((current) => !current)} aria-label={showConfirmation ? "Hide password" : "Show password"}>{showConfirmation ? <EyeOff size={17} /> : <Eye size={17} />}</button></div><small className="form-text d-block mb-3">Both passwords must match.</small><button className="btn btn-primary w-100" disabled={saving || !passwordValid}>{saving ? "Saving…" : "Change password"}</button></form>}</>}</div></div></div></div></main>;
}
